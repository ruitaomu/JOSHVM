...
then
	export TOOLCHAIN_HOME_DIR="/cygdrive/c/works/jfighter/NXP-RTT/rt-thread/env/tools/gnu_gcc/arm_gcc/mingw"
	echo TOOLCHAIN_HOME_DIR not specified using ${TOOLCHAIN_HOME_DIR}
fi

if [ -z "${RTTHREAD_HOME_DIR}" ]
then
	export RTTHREAD_HOME_DIR="C:/projects/bk7251_rtt_audio_sdk/rt-thread"
...
DIO_DIR=${WS_ROOT}/dio
PLATFORM_EXTRA_MAKEFILE_DIR=${WS_ROOT}/build_ext/board/bk7251_dev
...
if [ "${BUILD_CLDC}" = "true" ]
then
	BUILDMODULE=${CLDC_DIR}/build/javacall_arm_gcc
	cd ${BUILDMODULE}
	if [ "${CLEAN}" = "true" ]; then
		echo "Cleaning:    ${BUILDMODULE}"
...
make -f ${PLATFORM_EXTRA_MAKEFILE_DIR}/makefile \
	...
	CLDC_OUTPUT_DIR=${CLDC_DIR}/build/javacall_arm_gcc \
	PCSL_OUTPUT_DIR=${PCSL_OUTPUT_DIR} \
	JAVACALL_OUTPUT_DIR=${JAVACALL_OUTPUT_DIR}
