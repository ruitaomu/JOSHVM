...
pcsl_socket_open_start(unsigned char *ipBytes, int port, void **pHandle, void **pContext) {
	...
	if (JAVACALL_OK != javacall_socket_open(JAVACALL_IP_VERSION_4, &handle)) {
		return PCSL_NET_IOERROR;
	}

	*pHandle = handle;

    res = javacall_socket_connect_start(handle, JAVACALL_CONFIGURATION_NOT_SPECIFIED, JAVACALL_IP_VERSION_4, ipBytes, port, pContext);
...
pcsl_socket_open_finish(void *handle, void *context) {
    ...
    res = javacall_socket_connect_finish(handle, context);
...
pcsl_socket_close_start(void *handle, void **pContext) {
    ...
    res = javacall_socket_close_start(handle, JAVACALL_TRUE, pContext);

	javanotify_socket_event(JAVACALL_EVENT_SOCKET_CLOSE_COMPLETED, handle, JAVACALL_OK);
