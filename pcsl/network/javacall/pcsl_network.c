...
pcsl_network_getLocalIPAddressAsString(char *pLocalIPAddress) {
    ...
    res = javacall_network_get_local_ip_address_as_string(JAVACALL_IP_VERSION_4,pLocalIPAddress);
...
//    return javacall_to_pcsl_result(res);
      ...
      return PCSL_NET_IOERROR;     /** SHOULD NOT reach here! **/
}
#endif
...
pcsl_network_gethostbyname_start(char *hostname,
    ...
    res = javacall_network_gethostbyname_start(hostname,
        JAVACALL_IP_VERSION_4, JAVACALL_CONFIGURATION_NOT_SPECIFIED, pAddress, maxLen, pLen, pHandle, pContext);
...
pcsl_network_gethostbyname_finish(unsigned char *pAddress,
    ...
    res = javacall_network_gethostbyname_finish(JAVACALL_IP_VERSION_4, JAVACALL_CONFIGURATION_NOT_SPECIFIED, pAddress, maxLen, pLen, handle, context);
...
int pcsl_network_addrToString(unsigned char *ipBytes,
    ...
    temp = javacall_inet_ntoa (JAVACALL_IP_VERSION_4, ipBytes);
...
char * pcsl_inet_ntoa (void *ipBytes) {
    return javacall_inet_ntoa (JAVACALL_IP_VERSION_4, ipBytes);
}
