...
public final class SAX
{
	...
	final static private class Parser extends com.sun.ukit.xml.Parser
		implements
			...
			Locator
	{
		...
		{
			if (name.equals(SAX.FEATURE_NS)) {
				return mFNamespaces;
			} else if (SAX.FEATURE_PREF.equals(name)) {
				return mFPrefixes;
			} else if (SAX.FEATURE_ATTR2.equals(name)) {
				return true;
			} else {
			...
			throws SAXException, IOException
		{
			if ((mFNamespaces == true) && (mFPrefixes == false))
				...
				throw new SAXNotSupportedException(SAX.FEATURE_NS + SAX.FEATURE_PREF);
	...
	static final private class ParserImp extends javax.xml.parsers.SAXParser
	{
		...
		public boolean isNamespaceAware()
		{
			...
			try {
				awareness = parser.getFeature(SAX.FEATURE_NS);
			} catch (SAXException e) {
	...
	public final static class ParserFactoryImp 
		extends javax.xml.parsers.SAXParserFactory
	{
			...
			throws ParserConfigurationException, SAXException
		{
			...
			if ((namespaces == true) && (prefixes == false)) {
				parser.parser.setFeature(SAX.FEATURE_NS, true);
				parser.parser.setFeature(SAX.FEATURE_PREF, false);
			} else if ((namespaces == false) && (prefixes == true)) {
				parser.parser.setFeature(SAX.FEATURE_PREF, true);
				parser.parser.setFeature(SAX.FEATURE_NS, false);
			} else {
				throw new ParserConfigurationException("");
			}
			return parser;
