...
#define BREAK_INTERPRETER_LOOP(reason)                                \
  ...
  static void quick_native_method_entry(BasicType return_type) {
    ...
    jint r1, r2;
#ifdef XTENSA
	//Just an workaround for xtensa-gcc
	va_list empty_va_list;
#else
#define empty_va_list NULL
#endif
    invoke_native_entry(native_ptr, return_type, 0, empty_va_list, r1, r2);
...
static void init() {
  static int interpreter_table_initialized = 0;

  if (interpreter_table_initialized) return;
  else interpreter_table_initialized = 1;
...
void primordial_to_current_thread() {
  // In some cases, the Initer's constructor could not be invoked automatically,
  // because of compiler optimization. We have to manually invoke init() to
  // initialize interpreter_dispatch_table here;
  init();
