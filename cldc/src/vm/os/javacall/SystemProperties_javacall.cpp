...
#include "incls/_SystemProperties_javacall.cpp.incl"
...
int getInternalPropertyInt(const char* key) {
    ...
    if (JAVACALL_OK == javacall_get_property_int(key, JAVACALL_INTERNAL_PROPERTY, &value)) {
        return value;
    } else if (JAVACALL_OK == javacall_get_property_int(key, JAVACALL_APPLICATION_PROPERTY, &value)) {
        return value;
    }
