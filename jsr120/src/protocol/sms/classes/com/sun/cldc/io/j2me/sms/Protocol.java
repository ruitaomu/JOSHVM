...
package com.sun.cldc.io.j2me.sms;

import com.sun.cldc.io.j2me.ProtocolBase;
...
public class Protocol extends ProtocolBase {
	...
	try {
	    ...
	    SMSPacket smsPacket = new SMSPacket();
            ...
            length = receive0(m_iport, getSuiteID(), /**M@x: Use stub SuiteID**/
    ...
    public void close() throws IOException {
        /*
         * Set m_iport to -1, in order to quit out of the while loop
	...
	m_iport = -1;
        ...
        synchronized (closeLock) {
            if (open) {
				...
				if ((m_mode&Connector.READ) != 0) {
					//READ or READ_WRITE mode
					unblockRead = true;
					deRegister = 1;
				} else {
					deRegister = 0;
                ...
                close0(save_iport, connHandle, deRegister, unblockRead);
	...
	for (int i = 0; i < len; i++) {
	    if (!(openconnections.elementAt(i)
		  instanceof com.sun.cldc.io.j2me.sms.Protocol)
	...
	try {
            connHandle = open0(host, getSuiteID() /**M@x: Use stub SuiteID**/ , m_iport);
     ...
     * @param unblockRead if true, unblock any read pending threads
     * @return    0 on success, -1 on failure
     */
    private native int close0(int port, int handle, int deRegister, boolean unblockRead);
    ...
    protected int close00(int connHandle, int deRegister) {
        return close0(m_iport, connHandle, deRegister, (m_mode&Connector.READ)!=0?true:false);
    }
