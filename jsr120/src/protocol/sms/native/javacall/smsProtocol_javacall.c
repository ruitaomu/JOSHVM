                              ...
                              /* OUT */void **pContext)
{
	...
	int handle = 0;
	if (destPort == 0) {
		rtn = javacall_sms_send_noport( 0,
...
WMA_STATUS jsr120_remove_sms_listening_port(jchar smsPort) {
    javacall_result rtn = JAVACALL_OK;
...
void javanotify_incoming_sms(
									 javacall_int32 		 subscriber_slot,
									 javacall_sms_encoding	 msgType,
									 char*					 sourceAddress,
									 unsigned char* 		 msgBuffer,
									 int					 msgBufferLen,
	...
	SmsMessage* sms;
	unsigned char senderPhoneAddr[MAX_ADDR_LEN];
	int i = 0;
	REPORT_INFO(LC_WMA, "javanotify_incoming_sms...\n");
	memset(senderPhoneAddr, 0, MAX_ADDR_LEN);
	while ((i < MAX_ADDR_LEN) & (sourceAddress[i] != '\0')) {
		senderPhoneAddr[i] = sourceAddress[i];
		i++;
	}
	REPORT_INFO4(LC_WMA, "msgType=%d, sourceAddr=%s, sourcePort=%d, DestPort=%d\n", msgType, sourceAddress, sourcePortNum, destPortNum);
	sms = jsr120_sms_new_msg((javacall_sms_encoding)msgType, senderPhoneAddr, sourcePortNum, destPortNum, timeStamp, msgBufferLen, msgBuffer);
	if (sms) {
		jsr120_sms_pool_add_msg(sms); //javacall_event_send() will be called in this function.
	} /** If no space to store incoming sms, the message will be discarded. **/
}
 ...
 * TODO:
 *
 *      Send_Complete message will NOT send to VM, need to be processed in future!!!
 *
 **/
 	(void)result;
	(void)handle;
	REPORT_INFO(LC_WMA, "javanotify_sms_send_completed...\n");
 	return;
}
...
WMA_STATUS jsr120_number_of_sms_segments(unsigned char msgBuffer[], jint msgLen, jint msgType,
    ...
    *numSegments = javacall_sms_get_number_of_segments(
            ...
            (char*)msgBuffer,
            msgLen,
            (hasPort == KNI_TRUE) ? JAVACALL_TRUE: JAVACALL_FALSE);
...
WMA_STATUS jsr120_sms_unblock_thread(jint handle, jint waitingFor) {
    SNIReentryData rd;
	rd.waitingFor = (SNIsignalType)waitingFor;
	rd.pContext = NULL;
	rd.status = 0;
	rd.descriptor = handle;
	REPORT_INFO(LC_WMA, "In jsr120_sms_unblock_thread(), sending event...\n");
	javacall_event_send((unsigned char*)&rd, sizeof(SNIReentryData));
	//Currently, we use asynchronized method to unblock any possibly blocked thread,
	//so can't know if there's any thread really awaken. Simply return WMA_OK, which
	//means that always assume a thread is waiting for this signal and awaken.
	//So far, without the requirement of WMA Push supporting, everything OK.
	return WMA_OK;
}
