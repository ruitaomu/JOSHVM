...
#define MAX_SMS_MSG_LEN 150
typedef struct _SmsMessageBlocks {
	SmsMessage msg;
	int occupied;
	char msgAddr[MAX_ADDR_LEN];
	char msgBuffer[MAX_SMS_MSG_LEN];
} SmsMessageBlocks;

static SmsMessageBlocks message_blocks[MAX_SMS_MESSSAGES_IN_POOL];
static int _is_message_blocks_initialized = 0;
static SmsMessage* get_spare_sms_msg_block() {
	int i;
	if (!_is_message_blocks_initialized) {
		_is_message_blocks_initialized = 1;
		memset(message_blocks, 0, sizeof(message_blocks));
	}
	
	for (i = 0; i < MAX_SMS_MESSSAGES_IN_POOL; i++) {
		if (!message_blocks[i].occupied) {
			message_blocks[i].occupied = 1;
			message_blocks[i].msg.msgAddr = &message_blocks[i].msgAddr[0];
			message_blocks[i].msg.msgBuffer = &message_blocks[i].msgBuffer[0];
			return &message_blocks[i].msg;
		}
	}
	return NULL;
}

static void release_sms_msg_block(SmsMessage* sms) {
	((SmsMessageBlocks*)sms)->occupied = 0;
...
SmsMessage* jsr120_sms_new_msg(jchar	  encodingType,
	...
	if (msgLen > MAX_SMS_MSG_LEN) {
		return NULL;
	}
	
	SmsMessage *sms;
	if ((msgLen > MAX_SMS_MSG_LEN) || (NULL == (sms = get_spare_sms_msg_block()))) {
		return NULL;
	}
...
WMA_STATUS jsr120_sms_pool_delete_next_msg() {
    ...
    if (e) {
        SmsMessage* sms = (SmsMessage*)e->userData;
        jsr120_sms_delete_msg(sms);
        jsr120_sms_pool_decrease_count();
        jsr120_list_destroy(e);
        found = WMA_OK;
