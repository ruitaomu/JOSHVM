...
static ListElement _blocks[MAX_SMS_MESSSAGES_IN_POOL];
static int _is_list_initialized = 0;


static ListElement* get_spare_list_element() {
	int i;
	if (!_is_list_initialized) {
		_is_list_initialized = 1;
		memset(_blocks, 0, sizeof(_blocks));
	}
	
	for (i = 0; i < MAX_SMS_MESSSAGES_IN_POOL; i++) {
		if (!_blocks[i].occupied) {
			_blocks[i].occupied = 1;
			return &_blocks[i];
		}
	}
	return NULL;
...
static void release_list_element(ListElement* e) {
...
ListElement* jsr120_list_new_by_number(ListElement** head, jint num,
    ...
    ListElement* q      = (ListElement*) get_spare_list_element();
	if (q) {
	    q->next             = NULL;
	    q->id               = num;
	    q->msid         = msid;
