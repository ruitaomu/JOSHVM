...
void javacall_configdb_dump(javacall_handle config_handle, 
    ...
    res = javacall_file_open(unicodeFileName,
                             ...
                             &file_handle);
    if (res != JAVACALL_OK) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_configdb_dump(): ERROR - Can't open the dump file!\n");
        return;
    }
...
void javacall_configdb_dump_ini(javacall_handle config_handle, 
    ...
    res = javacall_file_open(unicodeFileName,
                             ...
                             &file_handle);
    if (res != JAVACALL_OK) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_configdb_dump_ini(): ERROR - Can't open the dump file!\n");
        return;
    }
...
javacall_handle configdb_load_no_fs () {
    ...
    for (i = 0; javacall_static_properties_sections[i] != NULL; i++) {        
		...
		//add machine-depended keys and values
		for (j = 0; javacall_static_properties_md_keys[i][j] != NULL; j++) {
            configdb_add_entry(db, javacall_static_properties_sections[i], javacall_static_properties_md_keys[i][j], 
                               javacall_static_properties_md_values[i][j]);
        }
    }
