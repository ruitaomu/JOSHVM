 ...
 */
javacall_result javacall_sms_send(  javacall_int32 subscriber_slot,
    ...
    char* IP_text = getProp("JSR_120_DATAGRAM_HOST", "192.168.1.204");
    ...
    if (ok == JAVACALL_OK) {
        ...
        if (ok != JAVACALL_OK) {
            javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_WMA, "Error: SMS sending - datagram blocked.\n");
        }
    }

    javacall_logging_printf(JAVACALL_LOGGING_INFORMATION, JC_WMA, "## javacall: SMS sending...\n");
...
javacall_result javacall_sms_is_service_available(javacall_int32 subscriber_slot){
    return JAVACALL_OK;
}
...
javacall_result javacall_sms_add_listening_port(unsigned short portNum){
    ...
    if (free == -1) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_WMA, "ports amount exceeded");
        return JAVACALL_FAIL;
    }
...
void javacall_sms_deinit(void) {}

javacall_result javacall_sms_send_noport(
               /* IN */ javacall_int32          subscriber_slot,
                        javacall_sms_encoding   msgType, 
                        const unsigned char*    destAddress, 
                        const unsigned char*    msgBuffer, 
                        int                     msgBufferLen,
                        int                     handle) {
     return javacall_sms_send(subscriber_slot, msgType, destAddress, msgBuffer, msgBufferLen, 0, 0, handle);
}
