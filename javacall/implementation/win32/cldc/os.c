...
struct _javacall_mutex* javacall_os_mutex_create() {
...
struct _javacall_cond* javacall_os_cond_create(struct _javacall_mutex *m) {
...
struct _javacall_mutex* javacall_os_cond_get_mutex(struct _javacall_cond *c) {
    assert(c != NULL);
    assert(c->mutex != NULL);
    return c->mutex;
}
...
void javacall_os_hardware_reset() {
	javacall_logging_printf(JAVACALL_LOGGING_CRITICAL, JC_NETWORK, "javacall_os_hardware_reset!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
	exit(1);
}
