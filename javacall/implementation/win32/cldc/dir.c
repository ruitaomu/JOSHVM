...
javacall_handle javacall_dir_open(const javacall_utf16* path,
  ...
  javacall_handle handle;
    ...
    if( pathLen > JAVACALL_MAX_FILE_NAME_LENGTH ) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_dir_open: Path length too long");
        return NULL;
    }
...
javacall_utf16* javacall_dir_get_next(javacall_handle handle,
                                        int* /*OUT*/ outFilenameLength, javacall_bool* isHidden,
       									javacall_bool* isDirectory) {

	DEBUG_PRINT("javacall_dir_get_next\n");
...
long internal_get_used_helper(const javacall_utf16* unicodeSystemDirName,
          ...
          for (; ; ) {
              if ((current = javacall_dir_get_next(pIterator, &currentLen, &isHidden, &isDirectory)) == NULL) {
                      break;
              }

              if( currentLen > JAVACALL_MAX_FILE_NAME_LENGTH ) {
                    javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "internal_get_used_helper: File name length too big");
...
javacall_result javacall_dir_get_root_path(javacall_utf16* /* OUT */ rootPath,
	...
	DEBUG_PRINT("javacall_dir_get_root_path\n");
    ...
    if (midpHome != NULL) {
        ...
        if (len >= *rootPathLen) {
            * rootPathLen = 0;
            javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_dir_get_root_path: MIDP_HOME "
                           "is too long.");
            return JAVACALL_FAIL;
        }
    ...
    if ( _wgetcwd( currDir, sizeof(currDir)/sizeof(wchar_t) ) == NULL) {
        * rootPathLen = 0;
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_dir_get_root_path: _wgetcwd failed");
        return JAVACALL_FAIL;
    } else {
        wchar_t* lastsep;
        ...
        if (j == 2) {
            javacall_logging_printf(JAVACALL_LOGGING_WARNING, JC_FILE, "Warning: cannot find appdb subdirectory.\n"
...
DEBUG_PRINT("javacall_get_file_separator\n");
...
javacall_bool javacall_dir_is_secure_storage(javacall_utf16* classPath,
	...
	DEBUG_PRINT("javacall_dir_is_secure_storage\n");
