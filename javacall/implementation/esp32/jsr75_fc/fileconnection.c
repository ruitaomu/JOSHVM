...
javacall_result javacall_fileconnection_dir_exists(const javacall_utf16* pathName,
    ...
    if(JAVACALL_OK != javacall_fileconnection_is_directory(pathName, pathNameLen, &res)) {
        javacall_logging_printf(JAVACALL_LOGGING_INFORMATION, JC_FC, "javacall_fileconnection_dir_exists(): not exist directory\n");
        return JAVACALL_FAIL;
    }
...
javacall_fileconnection_get_path_for_root(javacall_const_utf16_string rootName,
    ...
    static char* realroot_def[] = {JC_NATIVE_INTERNAL_ROOT_PATH,
                ...
                JC_NATIVE_USER_ROOT_PATH, "/sdcard/"};
    static char* root[] = {"internal/", "root/", "private/", "Phone/", "sdcard/"};
    static char* propname[] = {"internal_dir", "root_dir", "private_dir", "user_dir", "sdcard_dir"};
    int i, r;
    int len;
    int ne;
