...
javacall_result javacall_file_open(const javacall_utf16 * unicodeFileName, int fileNameLen, int flags, /*OUT*/ javacall_handle * handle) {
	...
	if (flags & JAVACALL_FILE_O_RDONLY) {
		native_flag |= O_RDONLY;
	}
	if (flags & JAVACALL_FILE_O_WRONLY) {
		native_flag |= O_WRONLY;
	}
	if (flags & JAVACALL_FILE_O_RDWR) {
		native_flag |= O_RDWR;
	}
	if (flags & JAVACALL_FILE_O_TRUNC) {
		native_flag |= O_TRUNC;
	}
	if (flags & JAVACALL_FILE_O_APPEND) {
		native_flag |= O_APPEND;
	}

	javacall_logging_printf(JAVACALL_LOGGING_INFORMATION, JC_FILE, "javacall_file_open: %s, flags=%d, native_flag=%d\n", pszOsFilename, flags, native_flag);

    int fd = open(pszOsFilename, native_flag, creationMode);
    javacall_logging_printf(JAVACALL_LOGGING_INFORMATION, JC_FILE, "file oepn returns: %d\n", fd);
...
javacall_result javacall_file_rename(const javacall_utf16 * unicodeOldFilename, int oldNameLen,
    ...
    char oldFileName[JAVACALL_MAX_FILE_NAME_LENGTH];
    char newFileName[JAVACALL_MAX_FILE_NAME_LENGTH];

    char* pszOldFilename = javacall_UNICODEsToUtf8(unicodeOldFilename, oldNameLen);
    if (pszOldFilename == NULL) {
        return JAVACALL_FAIL;
    }
    memcpy(oldFileName, pszOldFilename, oldNameLen * sizeof(char));
    ...
    memcpy(newFileName, pszNewFilename, newNameLen * sizeof(char));
