...
javacall_result jctest_time() {
	...
	if (JAVACALL_OK == javacall_time_initialize_timer(100, JAVACALL_FALSE, timer_cb, &handle1)
		&&
...
javacall_result jctest_event() {
	...
	javacall_printf("========jctest_event========[%d,%d]\n", javacall_time_get_milliseconds_since_1970() - starttime, timer_count2);
...
javacall_result jctest_network_ex() {
	...
	res = javacall_network_gethostbyname_start(hostname, JAVACALL_IP_VERSION_4, 0, host, 16, &len, &ah, &pcontext);
	if (JAVACALL_WOULD_BLOCK == res) {
	...
	res = javacall_network_gethostbyname_finish(JAVACALL_IP_VERSION_4, 0, host, 16, &len, ah, pcontext);
	javacall_printf("javacall_network_gethostbyname_start: host = %s\n", inet_ntoa(host));
