...
static void normalize_dir_name(char* szDirname, int dirNameLen) {
    if (dirNameLen > 0 && szDirname[dirNameLen - 1] == '/') {
        szDirname[dirNameLen - 1] = '\0';
    }
}
...
javacall_result javacall_fileconnection_is_readable(const javacall_utf16* pathName,
    ...
    if (FR_OK != f_stat (javacall_UNICODEsToUtf8(pathName, pathNameLen), &info)) {
        return JAVACALL_FAIL;
    }
...
javacall_result javacall_fileconnection_is_directory(const javacall_utf16* pathName,
                                                     ...
                                                     javacall_bool* /* OUT */ result) {                                   
    FILINFO info;
    char* szPathname = javacall_UNICODEsToUtf8(pathName, pathNameLen);
    normalize_dir_name(szPathname, pathNameLen);
    
    if (FR_OK != f_stat(szPathname, &info)) {
        return JAVACALL_FAIL;
    }
...
javacall_result javacall_fileconnection_create_dir(const javacall_utf16* dirName,
    ...
    char* szDirname = javacall_UNICODEsToUtf8(dirName, dirNameLen);
    normalize_dir_name(szDirname, dirNameLen);
    
    if(FR_OK != f_mkdir(szDirname)) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FC,  
        "Error: javacall_fileconnection_create_dir(), cannot create directory %s\n", szDirname);
        return JAVACALL_FAIL;
    }
    return JAVACALL_OK;
}
...
javacall_result javacall_fileconnection_delete_dir(const javacall_utf16* dirName,
    ...
    if(FR_OK != f_unlink(szDirname)) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FC,  
        "Error: javacall_fileconnection_delete_dir(), cannot delete directory %s\n", szDirname);
        return JAVACALL_FAIL;
    }
    return JAVACALL_OK;
...
javacall_result javacall_fileconnection_rename_dir(const javacall_utf16* oldDirName,
    ...
    char* szDirnameOld = javacall_UNICODEsToUtf8(oldDirName, oldDirNameLen);
    char* szDirnameNew = javacall_UNICODEsToUtf8(newDirName, newDirNameLen);
    normalize_dir_name(szDirnameOld, oldDirNameLen);
    normalize_dir_name(szDirnameNew, newDirNameLen);
    
    if(FR_OK != f_rename(szDirnameOld, szDirnameNew)) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FC,  
        "Error: javacall_fileconnection_rename_dir(), cannot rename directory %s to %s\n", szDirnameOld, szDirnameNew);
        return JAVACALL_FAIL;
    }
    return JAVACALL_OK;
