 ...
 *
 * @param ip_version the IP version specification. Possible values are:
 *  JAVACALL_IP_VERSION_4, JAVACALL_IP_VERSION_6
 ...
 * @return JAVACALL_OK if was successfully created, false otherwise
 */
 ...
 *
 * @param handle handle of the socket to connect.
 * @param config_id the Id of the connection configuration used
 *        to bind to, possible value JAVACALL_CONFIGURATION_NOT_SPECIFIED.
 * @param ip_version the IP version specification. Possible values are:
 *  JAVACALL_IP_VERSION_4, JAVACALL_IP_VERSION_6
 * @param pAddress The IP address of the remote device in the form of byte array.
 *        Should contain 4 bytes if it's an IPv4 address,
 *        16 bytes if it is an IPv6 address
 ...
 * @retval JAVACALL_WOULD_BLOCK  if the caller must call the finish function to
 *  complete the operation
 * @retval JAVACALL_CONNECTION_NOT_FOUND when there was some other error
 *  (Connection not found exception case)
 */
javacall_result javacall_socket_connect_start(javacall_handle handle,
 ...
 *  (Connection not found exception case)
 */
javacall_result javacall_socket_connect_finish(javacall_handle handle,
                                            void *context);
 ...
 *        set only when this function returns JAVACALL_OK. 0 is returned
 ...
 *
 * @param handle handle of an open connection
 * @param abort  whether the open connection must be aborted or not
                 JAVACALL_TRUE - connection must be aborted, operation can't be blocking
                 JAVACALL_FALSE - connection can be closed using standard TCP close procedure, blocking is allowed
 * @param pContext address of a pointer variable to receive the context;
 *	  it is set only when this function returns JAVACALL_WOULDBLOCK
 *
 * @retval JAVACALL_OK          success
 ...
 ******************************************************************************
  ...
  - - - -  - - - - - - -  
  The following functions are implemented in CLDC.
...
typedef enum {
    ...
    JAVACALL_EVENT_SOCKET_CONNECT_COMPLETED         =1000,
    ...
    JAVACALL_EVENT_NETWORK_GETHOSTBYNAME_COMPLETED  =1004,
    /** Get host by address completed */
    JAVACALL_EVENT_NETWORK_GETHOSTBYADDR_COMPLETED  =1005,
    /** Accept completed */
    JAVACALL_EVENT_SERVER_SOCKET_ACCEPT_COMPLETED   =2000
} javacall_socket_event_type;
 ...
 *          - JAVACALL_EVENT_NETWORK_GETHOSTBYADDR_COMPLETED
 ...
 */
void javanotify_socket_event(javacall_socket_event_type type,
 ...
 *
 * @param config_id the Id of the connection configuration used to establish
 *   connection, possible to pass JAVACALL_CONFIGURATION_NOT_SPECIFIED.
 * @param ip_version the IP version specification.
 *  May be JAVACALL_IP_VERSION_ANY. It means that function tries to open
 *  server socket which will listen connections from either IPv4 or IPv6
 *  clients(if it's possible). On dual-stack systems it will create an IPv6
 *  socket. On separate-stack systems will create two sockets, one for IPv4
 *  and one for IPv6 communication.
 *  It's not guaranteed that both IP versions will be available. For
 *  JAVACALL_IP_VERSION_ANY this function returns JAVACALL_OK if server socket
 *  created at least for one of IP version.
...
javacall_result /*OPTIONAL*/ javacall_server_socket_open_start(
                                    javacall_configuration_id config_id,
                                    javacall_ip_version ip_version,
                                    int port, javacall_handle *pHandle,
                                    void **pContext);
 ...
 * @retval JAVACALL_WOULD_BLOCK  if the caller must call the finish function again to complete the operation
 */
javacall_result /*OPTIONAL*/javacall_server_socket_open_finish(void *handle,
                                                               void *context);
