...
/** 
 * Returns the mounted root file systems on external memory devices. Each root
 * must end with '/' character.
 *
 * @param roots buffer to store the string containing
 *              currently mounted external roots separated by ';' character.
 * @param rootsLen available buffer size (maximum number of
 *                 characters to be stored).
 * @param fromCache indicates whether the returned value should be taken from
 *                  internal cache (this parameter can be ignored if properties
 ...
 */
javacall_result
javacall_fileconnection_get_external_mounted_roots(
        javacall_utf16_string /* OUT */ roots, int rootsLen,
