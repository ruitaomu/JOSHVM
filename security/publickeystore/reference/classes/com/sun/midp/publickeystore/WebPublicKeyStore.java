...
public class WebPublicKeyStore extends PublicKeyStore
    ...
    /**
     * Load the certificate authorities for the application from storage
     * into the SSL keystore.
     */
    public static void loadCertificateAuthorities() {
        Protocol storage = null;
        InputStream tks = null;
        WebPublicKeyStore ks = null;
        ...
        try {
            if (keystoreLocation != null ) {
				...
				storage.openPrim(WebPublicKeyStore.securityToken, keystoreLocation, Connector.READ_WRITE, false);
				tks = storage.openInputStream();
            } else if (appKeystoreLocation != null) {
            	tks = clazz.getResourceAsStream(appKeystoreLocation);
            } else {
                throw new Exception("Unassigned keystoreLocation");
            }
        } catch (Exception e) {
            if (Logging.TRACE_ENABLED) {
                Logging.trace(e, "Could not open the trusted key store, " +
                              "cannot authenticate servers");
            }
        }
    ...
    private static void saveKeyList() {
        ...
        FileConnection storage;
        OutputStream outputStream;

        if ((trustedKeyStore == null) || (sharedKeyList == null)) {
            return;
        }
        ...
        try {
            if (keystoreLocation != null ) {
                storage = (FileConnection)Connector.open("file://" + keystoreLocation);
            } else {
                throw new Exception("Unassigned keystoreLocation");
            }
    ...
    public static void initKeystoreLocation(SecurityToken securityToken, String location) {
        ...
        if ((keystoreLocation == null) && (appKeystoreLocation == null)) {
            keystoreLocation = location;
			WebPublicKeyStore.securityToken = securityToken;
        }
    }

	public static void initKeystoreLocation(Class classToResource, String pathToResource) {

        if ((keystoreLocation == null) && (appKeystoreLocation == null)) {
			clazz = classToResource;
            appKeystoreLocation = pathToResource;
        }
    }

    private static String keystoreLocation;
	private static String appKeystoreLocation;
