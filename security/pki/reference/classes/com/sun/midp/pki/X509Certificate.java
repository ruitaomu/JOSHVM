...
public class X509Certificate implements Certificate {
	...
	/**
     * Expected prefix in decrypted value when SHA-256 hash is used for signing
    ...
    private BouncyCastleRSAPublicKey pubKey = null;
    ...
    public X509Certificate(byte ver, byte[] rawSerialNumber, String sub,
        ...
        throws Exception {
            ...
            pubKey = new BouncyCastleRSAPublicKey(BigIntegers.fromUnsignedByteArray(mod), BigIntegers.fromUnsignedByteArray(exp));
        ...
        while (idx < end) {
            ...
            if ((enc[extIdIdx] == 0x55) && (enc[extIdIdx + 1] == 0x1d)) {
                ...
                case 0x11:   // subAltName = id-ce 17
					...
					int totalLength = getLen(SEQUENCE_TYPE);
                     ...
                     */
                    while (totalLength > 0) {
                        Object subAltName = null;
                        StringBuffer temp = new StringBuffer();
                        byte subAltNameType =
                                (byte) (enc[idx] - 0x80);
                        int valueLen = getLen(enc[idx]);
                        int start = idx;						
                        ...
                        switch (subAltNameType) {
							...
							default:
								if (crit) {
									throw new IOException("Unsupported critical subAltName type " + crit);
								}
    ...
    public static X509Certificate generateCertificate(byte[] buf, int off,
        ...
        try {
            ...
            if (id != RSA_ENCRYPTION) {
                ...
                res.pubKey = new BouncyCastleRSAPublicKey(BigIntegers.fromUnsignedByteArray(res.enc, modulusPos, modulusLen),
    ...
    public void verify(PublicKey k) throws CertificateException {
        ...
        try {
            RSAKeyParameters key = new RSAKeyParameters(false, pk.getModulus(), pk.getPublicExponent());

            AsymmetricBlockCipher rsa = new RSAEngine();
			rsa = new PKCS1Encoding(rsa);
			rsa.init(false, key);
            result = rsa.processBlock(signature, 0, signature.length);
			val = result.length;
        } catch (Exception e) {
            throw new CertificateException(this,
                CertificateException.VERIFICATION_FAILED);
         ...
         *
         * For SHA-256, the 32-byte hash will be preceded by
         * 3031300d060960864801650304020105000420
         * 30 31       49: SEQUENCE
         * 30 0d       13: . SEQUENCE
         * 06 09        9: . . OID  2.16.840.1.101.3.4.2.1 (SHA-256 digest OID)
         *              0: 60 86 48 01 65 03 04 02 01
         * 05 00        0: . . NULL (null parameters)
         * 04 20       32: . <32-byte hash>
         */ 
        if ((sigAlg == MD2_RSA) &&
		...
		if ((sigAlg == SHA256_RSA) &&
            (val == (PREFIX_SHA256.length + TBSCertHash.length)) &&
            Utils.byteMatch(result, 0, 
                            PREFIX_SHA256, 0, PREFIX_MD5.length) &&
            Utils.byteMatch(result, PREFIX_SHA256.length,
                            TBSCertHash, 0, TBSCertHash.length)) {
            return;
        }
